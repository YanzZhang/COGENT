...
/* We do not support C11 <threads.h>.  */
     ...
     & ibmaglo3:ibmaghi3)
      ...
      REAL*8 p
      integer i,j,k,l, n
      double precision b, v_r, cos_theta, Y0, mult_coeff, mu_coeff_HiFac
     &e
     ...
     & irhslo3:irhshi3)
      ...
      integer imcoeff_rholo0,imcoeff_rholo1,imcoeff_rholo2,imcoeff_rholo
     &3
      integer imcoeff_rhohi0,imcoeff_rhohi1,imcoeff_rhohi2,imcoeff_rhohi
     &3
      REAL*8 mcoeff_rho(
     ...
     & 0:nmcoeff_rhocomp-1)
      ...
      integer imcoeff_philo0,imcoeff_philo1,imcoeff_philo2,imcoeff_philo
     &3
      integer imcoeff_phihi0,imcoeff_phihi1,imcoeff_phihi2,imcoeff_phihi
     &3
      REAL*8 mcoeff_phi(
     ...
     & ibmaglo3:ibmaghi3)
      ...
      REAL*8 p
      integer i,j,k,l, n
      double precision b, v_r, cos_theta, Y0, mult_coeff_phi, mult_coeff
     &_rho, mu_coeff_HiFace
      double precision vpar_max, vperp_max, inner_part, outer_part, vr_s
     &ep
      do l = igridboxlo3,igridboxhi3
      do k = igridboxlo2,igridboxhi2
      do j = igridboxlo1,igridboxhi1
      do i = igridboxlo0,igridboxhi0
         if ((k.eq.-Nvpar/2).or.(k.eq.Nvpar/2-1)) then
           ...
           do n = 0, nmcoeff_phicomp-1
              mult_coeff_phi = mcoeff_phi(i,j,imcoeff_philo2,imcoeff_phi
     &lo3,n)
              mult_coeff_rho = mcoeff_rho(i,j,imcoeff_philo2,imcoeff_phi
     &lo3,n)
              inner_part = 1.0/(v_r**(n+1))/(2.0*n+1.0)* (-mult_coeff_ph
     &i)
              outer_part = 0.5*(1/v_r**(n-1) - vr_sep**2/v_r**(n+1) + 2.
     &0/v_r**(n-1)/(2.0*n-1.0)) * (mult_coeff_rho) /(2.0*n+1.0)**2
              rhs(i,j,k,l) = rhs(i,j,k,l)
     ...
     & ibmaglo3:ibmaghi3)
      ...
      double precision mu_se, mu_be, vpar_se, vpar_be, area_coeff, vpar_
     &global_max, vperp_global_max
      do l = igridboxlo3,igridboxhi3
      do k = igridboxlo2,igridboxhi2
      do j = igridboxlo1,igridboxhi1
      do i = igridboxlo0,igridboxhi0
        ...
        if (vr_sep.gt.vr_max) then
         ...
         if (bnd_index_se.ne.bnd_index_be) then
            if (mu_se.gt.mu_min) then
              area_coeff=1.0-0.5/(dx(0)*dx(1))*(mu_be-mu_se)*(vpar_be-vp
     &ar_se)
            else
              area_coeff =0.5/(dx(0)*dx(1))*(mu_be-mu_se)*(vpar_be-vpar_
     &se)
            endif
         else
     ...
     &)
              ...
              area_coeff = 0.5*(vpar_be-vpar_se)/dx(0) + (vpar_max-vpar_
     &be)/dx(0)
            endif
         endif
     ...
     & ibmaglo3:ibmaghi3)
      ...
      do i = igridboxlo0,igridboxhi0
        ...
        do n = 0, nkernelcomp-1
          kernel(i,j,k,l,n) = scale_factor * v_r**n * Y0(cos_theta,n) * 
     &rho(i,j,k,l)
        enddo
      enddo
      enddo
      ...
      SELECT CASE (n)
         ...
         CASE(8)
            Y0=1.0/128.0*(6435.0*x**8-12012.0*x**6+6930.0*x**4-1260.0*x*
     &*2+35.0)
      END SELECT
