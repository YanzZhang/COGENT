...
FokkerPlanck::FokkerPlanck( const std::string& a_ppcls_str, const int a_verbosity, const bool a_like_species )
   : m_verbosity((const bool)a_verbosity),
     ...
     m_subtract_background(a_like_species),
     m_ref_updated(false),
                                ...
                                const KineticSpeciesPtrVect& a_soln_physical )
{
  ...
  if(!m_energy_cons.isDefined()){
   ...
   //compute the constant Cls coefficients in the Pretimestep
    computeClsNorm(m_cls_norm, soln_species_m_a.mass(), soln_species_m_a.charge(),soln_species_m_b.charge());
  }
  ...
  computePotentialsAndCoeffs( soln_species_m_b, dfn_m_b, grids_m_b, phase_geom_ptr_bkgr,soln_species_m_a.mass());
...
void FokkerPlanck::printParameters()
{
  if (procID()==0) {
    ...
    if ((m_subtract_background)) {
      ...
      std::cout<< "Fokker-Planck: C(F0_a,F0_b) is set to zero because of subtract_background is set to ture" <<std::endl;
      if(!m_compute_maxwellian){
    ...
    if (m_subtract_background) {
      ...
      if (m_ref_updated || m_first_call_pc) {
        if (!procID() && m_verbosity) {
          std::cout<<"  Note: Reference distribution function updated.\n";
                                ...
                                const Real                    a_time )
{
  ...
  if (!m_rosenbluth_skip_stage) {
    ...
    computePotentialsAndCoeffs(soln_species_bkgr, dfn_bkgr, grids_bkgr, phase_geom_ptr_bkgr,soln_species.mass());
