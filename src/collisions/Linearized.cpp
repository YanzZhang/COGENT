...
// NB: a_soln is on the computational grid and has 4 ghost cells (passed here as Nans)
{
   ...
   if (m_first_step) {
    ...
    if (!m_fixed_cls_freq){
      ...
      computeSelfConsistFreq(m_sc_cls_freq, inj_density, soln_species.mass(), soln_species.charge(),soln_species_bkgr.charge());
    }
   ...
   for (DataIterator sdit(soln_dfn.dataIterator()); sdit.ok(); ++sdit) {
    ...
    if(a_species==a_species_bkgr)  delta_dfn[sdit].minus( init_dfn[sdit] ); //full f_a is used for unlike-species}
  }

   //Covert delta_dfn to cell centers (original data is face-centered? )
   convertToCellCenters(phase_geom, delta_dfn);


   // Calculate test-particle (TP) collisional RHS
   const double mass = soln_species.mass();
   const double mass_bkgr = soln_species_bkgr.mass();
   LevelData<FArrayBox> tp_rhs_coll(grids, n_comp, IntVect::Zero);
   testPartCollRHS(tp_rhs_coll, delta_dfn, phase_geom, mass,mass_bkgr);
...
void Linearized::testPartCollRHS(LevelData<FArrayBox>& a_rhs_coll,
                                 ...
                                 const double a_mass,
                                 const double a_mass_bkgr ) const
{
   ...
   for (DataIterator dit( a_rhs_coll.dataIterator() ); dit.ok(); ++dit) {
      ...
      FORT_EVALUATE_TP_LORENTZ_SPECIES(CHF_FRA(this_flux_cell),
                               ...
                               CHF_CONST_REAL(a_mass_bkgr));

      //Add energy-diffusion part of the TP collisions
      FORT_EVALUATE_TP_ENERG_DIFF_SPECIES(CHF_FRA(this_flux_cell),
                               CHF_CONST_FRA1(this_delta_dfn_tmp,0),
                               CHF_CONST_FRA1(this_b,0),
                               CHF_CONST_FRA1(this_temperature,0),
                               CHF_BOX(this_flux_cell.box()),
...

{
    ...
    double cls_norm = coeff * N * pow(ech, 2) * pow(a_charge, 2) *pow(a_charge_bkgr,2)* L
