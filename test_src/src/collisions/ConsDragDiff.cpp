                               ...
                               const Real                    time )
{
  ...
  */
      ...
      CFG::LevelData<CFG::FArrayBox> dens(mag_geom.grids(), 1, CFG::IntVect::Zero);
      CFG::LevelData<CFG::FArrayBox> Ushift(mag_geom.grids(), 1, CFG::IntVect::Zero);
      soln_species.numberDensity(dens);
      soln_species.ParallelMomentum(Ushift);
      CFG::DataIterator cfg_dit = Ushift.dataIterator();
...
inline void ConsDragDiff::printParameters( const KineticSpeciesPtrVect&  soln )
{
   ...
   CFG::LevelData<CFG::FArrayBox> temp_cfg(mag_geom.grids(), 1, CFG::IntVect::Zero);
