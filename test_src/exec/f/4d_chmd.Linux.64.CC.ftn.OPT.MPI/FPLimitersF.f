...
/* We do not support C11 <threads.h>.  */
     ...
     & iulo3:iuhi3)
      ...
      integer Nvpar
        ...
        DOUBLE PRECISION, PARAMETER :: flc_fac = 0.25, one_by_eight = 0.
     &125,
     ...
     & one_by_twenty = 1.0_8/20.0_8
      ...
      do i = igridlo0,igridhi0
            ...
            IF (a(i,j,k,l) .LT. 0.0_8) THEN
              fu = u(i,j,k-1,l)
              fd = u(i,j,k,l)
              fq = fc - one_by_eight * (u(i,j,k-2,l)+u(i,j,k,l)-2.0_8*u(
     &i,j,k-1,l))
            ELSE
              fu = u(i,j,k,l)
              fd = u(i,j,k-1,l)
              fq = fc - one_by_eight * (u(i,j,k+1,l)+u(i,j,k-1,l)-2.0_8*
     &u(i,j,k,l))
            ENDIF
            f3 = fu + flc_fac * (fc - fu)
     ...
     & one_by_twenty = 1.0_8/20.0_8
      ...
      do i = igridlo0,igridhi0
            ...
            IF (a(i,j,k,l) .LT. 0.0_8) THEN
              fu = u(i,j,k,l-1)
              fd = u(i,j,k,l)
              fq = fc - one_by_eight * (u(i,j,k,l-2)+u(i,j,k,l)-2.0_8*u(
     &i,j,k,l-1))
            ELSE
              fu = u(i,j,k,l)
              fd = u(i,j,k,l-1)
              fq = fc - one_by_eight * (u(i,j,k,l+1)+u(i,j,k,l-1)-2.0_8*
     &u(i,j,k,l))
            ENDIF
            f3 = fu + flc_fac * (fc - fu)
            f(i,j,k,l) = med(fc,med(fc,fd,f3),fq)
